...
class Ant:
    ...
    It follows the normal Ant Path Calculations with a bias toward key nodes.
    """
    ...
    def select_next_node(self, current_node, nodes_to_visit=None):
        ...
        """
        #Get neighbors range from CSR indptr
        ...
        #Slice arrays to get neighbors data
        ...
        #Select possible neighbors
        if not np.any(valid_mask):
            # All neighbors visited: Consider ALL of them anyway
    ...
    def calculate_path(self, starting_node, log_print=False, TSP=False):
        ...
        if TSP:
            while current_node != starting_node:
                next_node = self.select_next_node(current_node, {starting_node})
...
def two_opt_csr(path_array, indptr, indices, costs, max_iterations=3, window_size=30):
    """
    Classical 2-opt optimization using CSR cost lookups.
    """
    n = len(path_array)
    improved = True
    iterations = 0
