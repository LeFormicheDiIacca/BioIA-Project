...
def print_gen_log(gen, nevals, record, num_dead, duration, is_header=False):
    """
    """
    header = f"{'Gen':>4} | {'Nevals':>6} | {'Avg Fit':>12} | {'Min Fit':>12} | {'Max Fit':>12} | {'Dead':>5} | {'Time':>7}"
    csv_path = os.path.join(BASE_FOLDER, "evolution_stats.csv")
    if is_header:
        print("-" * len(header))
        print(header)
        print("-" * len(header))
        csv_headers = ["gen", "nevals", "avg", "min", "max", "dead", "time"]
    ...
    print(f"{gen:>4} | {nevals:>6} | {avg_str} | {min_str} | {max_str} | {num_dead:>5} | {duration:>6.2f}s")
    with open(csv_path, 'a', newline='') as f:
        csv.writer(f).writerow([gen, nevals, avg_str, min_str, max_str, num_dead, duration])
...
# --- VECCHIA FUNZIONE PENALITA' NUMBA (aggiornata ai 3 array) ---
...
def compute_total_penalty_numba(predecessors, end_nodes, start_node_idx,
    ...
    for end_idx in end_nodes:
        ...
        while curr != start_node_idx:
            ...
            # Leggiamo direttamente dai 3 array separati!
...
# --- EVALUATE CON DIJKSTRA SCIPY ---
def evaluate_fully_optimized(individual, sources_list, targets_list, num_scenarios, edge_features_columns, csr_template,
                             csr_components):
    global _GLOBAL_PSET
    ...
    try:
        ...
        # Evitiamo costi negativi o nulli che rompono Dijkstra
        costs = np.maximum(costs, 0.001)
    except Exception as e:

        return (1e12,)  # Solo se l'equazione matematicamente esplode (es. divisione per zero non protetta)

    # 3. Aggiorna CSR Matrix
    ...
    # 5. Calcolo penalità reale sul percorso
    total_penalty = 0.0
...
def run_EA(scenarios_number, population, generations, res, npz_path):
    ...
    # HEADER TABELLA (Aggiunta colonna "Dead")
...
if __name__ == "__main__":
    ...
    for experiment in experiments:
        ...
        run_EA(scenarios_number=scen_number, population=population,
               generations=gens, res=res, npz_path=npz_path)
